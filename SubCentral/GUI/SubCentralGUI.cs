    ...
    public class SubCentralGUI : GUIWindow {
        ...
        public override bool OnMessage(GUIMessage message) {
            ...
            if (message.Message == GUIMessage.MessageType.GUI_MSG_USER) {
                if (message.TargetWindowId == SubCentralGUIID) {
        ...
        private void AddTemporaryCustomPluginHandler(int ID, List<string> pluginData) {
            TemporaryCustomPluginHandler customTemporaryPluginHandler = core.PluginHandlers[PluginHandlerType.CUSTOM] as TemporaryCustomPluginHandler;
        ...
        private void countDownloads(List<SubtitleDownloadStatus> statusList, out int succesful, out int canceled, out int errors) {
            ...
            foreach (SubtitleDownloadStatus subtitleDownloadStatus in statusList) {
                if (subtitleDownloadStatus.Succesful)
                    succesful++;
                else if (subtitleDownloadStatus.Canceled)
                    canceled++;
                else
                    errors++;
            }
        ...
        private bool CheckAndTranslateSkin() {
            if (cancelButton != null && languagesButton != null && modifySearchButton != null && //sortButton != null &&
                providerList != null &&
